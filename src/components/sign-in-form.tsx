  ...
  return (
    <div className="flex flex-1 items-center justify-center bg-secondary/40 px-4">
      ...
      >
        ...
        <motion.div whileTap={microsoftConfigured ? { scale: 0.98 } : undefined}>
          <Button
            size="lg"
            className="w-full gap-2"
            onClick={handleSignIn}
            disabled={pending || !microsoftConfigured}
          >
            <MicrosoftMark />
        ...
        {!microsoftConfigured ? (
          <p className="mt-4 text-center text-xs text-muted-foreground">
            Microsoft sign-in isn&apos;t configured on this deployment yet — the Azure app
            registration values are missing.
          </p>
        ) : error ? (
